...
MessageDlg('Você não pode alterar a data quanto tiver colocando itens ao pedido!',MtWarning,[mbok],0)
end;
...
begin
messageDlg('Não hà registros a serem deletados!',MtError,[mbok],0);
exit;
end;
...
if tbl_PedTotal.FieldByName('Status_PedTotal').Value = 'FINALIZADO' then
begin
MessageDlg('Pedido está finalizado!',MtWarning,[mbok],0);
exit;
end;
if tbl_PedTotal.RecordCount = 0 then
begin
MessageDlg('Não a nenhum pedido para ser finalizado!',MtWarning,[mbok],0);
exit;
end;
...
if cbCliente.Text = 'ESCOLHA O CLIENTE DESEJADO' then
begin
MessageDlg('Escolha um Cliente antes de fazer a importação dos dados!',MtWarning,[mbok],0);
exit;
end;
usuario := nome;
with OpenDialog1 do
