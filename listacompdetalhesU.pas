...
if cbFormPag.Text = 'FORMA DE PÁGAMENTO' then
begin
MessageDlg('Escolha a forma de págamento!',MtWarning,[mbok],0);
exit;
end;
...
cbFormPag.Text := 'Forma de págamento';
...
begin
ShowMessage('Ouve um problema com a descrição do produto '+descricao+', por favor contate o suporte!');
end;
...
MessageDlg('Não tem nenhum registro a ser apagado, essa venda será deletada!',MtError,[mbok],0);
...
begin
MessageDlg('Clique em gravar para continuar, caso contrario o estoque não será atualizado!',MtInformation,[mbok],0);
end;
...
begin
if MessageDlg('Você ainda tem uma COMPRA em aberto. Deseja cancelar?',MtInformation,[mbyes,mbno],0)=mryes then
begin
inserirqtd := 0;
tbl_CompTotal.Locate('Cod_CompTotal',IntToStr(inicio),[]);
edit := 0;
...
MessageDlg('As alterações ao registro foram salvas com sucesso!',MtInformation,[mbok],0);
frm_ListaCompDetalhes.Close;
frm_ListaCompDetalhes := Tfrm_ListaCompDetalhes.Create(Self);
frmPrincipal.Hide;
frmPrincipal.Show;
...
dia := formatdatetime('dd',strtodatetime(dia));
  ...
  begin
    ...
    FieldByName('Fornecedor_ContasPagar').Value := cbFornecedores.Text;
      ...
      if editSituacao.Text <> 'PAGO' then
      begin
        FieldByName('Pago_ContasPagar').Value := 'NÃO';
      end;
...
begin
  ...
  begin
    if (MessageDlg('Deseja realmente sair sem salvar as alterações?',mtConfirmation,[mbyes,mbno],0)=mryes)then
    begin
      for i:=1 to tbl_Compras.RecordCount do
       begin
        tbl_Compras.Delete;
