object frm_NewMovimentacao: Tfrm_NewMovimentacao
  Left = 357
  Top = 114
  BorderStyle = bsDialog
  Caption = 'Movimenta'#231#227'o Financeira'
  ClientHeight = 514
  ClientWidth = 779
  Color = clBtnFace
  Font.Charset = ANSI_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'Trebuchet MS'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  PixelsPerInch = 96
  TextHeight = 18
  object PageControl1: TPageControl
    Left = 0
    Top = 0
    Width = 779
    Height = 514
    Cursor = crHandPoint
    ActivePage = TabSheet1
    Align = alClient
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Trebuchet MS'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    OnDrawTab = PageControl1DrawTab
    object TabSheet1: TTabSheet
      BorderWidth = 1
      Caption = 'Geral'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Trebuchet MS'
      Font.Style = []
      ParentFont = False
      object Label1: TLabel
        Left = 42
        Top = 37
        Width = 71
        Height = 18
        Caption = 'Movimento:'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Trebuchet MS'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label2: TLabel
        Left = 439
        Top = 37
        Width = 109
        Height = 18
        Caption = 'Data de Cadastro:'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Trebuchet MS'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object editData: TDateTimePicker
        Left = 551
        Top = 32
        Width = 186
        Height = 26
        Date = 44082.525722407410000000
        Time = 44082.525722407410000000
        Enabled = False
        TabOrder = 0
        OnEnter = editDataEnter
        OnExit = editDataExit
      end
      object GroupBox1: TGroupBox
        Left = 32
        Top = 72
        Width = 705
        Height = 193
        ParentBackground = False
        TabOrder = 1
        object Label3: TLabel
          Left = 27
          Top = 28
          Width = 73
          Height = 18
          Caption = 'Documento:'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Trebuchet MS'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label4: TLabel
          Left = 40
          Top = 68
          Width = 60
          Height = 18
          Caption = 'Hist'#243'rico:'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Trebuchet MS'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label5: TLabel
          Left = 288
          Top = 101
          Width = 148
          Height = 22
          Caption = 'Valor do Movimento:'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -16
          Font.Name = 'Trebuchet MS'
          Font.Style = []
          ParentFont = False
        end
        object editDocumento: TEdit
          Left = 104
          Top = 24
          Width = 273
          Height = 24
          CharCase = ecUpperCase
          Ctl3D = False
          ParentCtl3D = False
          TabOrder = 0
          OnEnter = editDocumentoEnter
          OnExit = editDocumentoExit
        end
        object editHistorico: TEdit
          Left = 104
          Top = 64
          Width = 569
          Height = 24
          CharCase = ecUpperCase
          Ctl3D = False
          ParentCtl3D = False
          TabOrder = 1
          OnEnter = editHistoricoEnter
          OnExit = editHistoricoExit
        end
        object rbDebito: TRadioButton
          Left = 104
          Top = 104
          Width = 73
          Height = 17
          Caption = 'D'#233'bito'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Trebuchet MS'
          Font.Style = [fsBold]
          ParentFont = False
          TabOrder = 2
          OnClick = rbDebitoClick
        end
        object rbCredito: TRadioButton
          Left = 168
          Top = 104
          Width = 73
          Height = 17
          Caption = 'Cr'#233'dito'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Trebuchet MS'
          Font.Style = [fsBold]
          ParentFont = False
          TabOrder = 3
          OnClick = rbCreditoClick
        end
        object EditValor: TEdit
          Left = 288
          Top = 122
          Width = 174
          Height = 30
          Ctl3D = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -19
          Font.Name = 'Trebuchet MS'
          Font.Style = []
          ParentCtl3D = False
          ParentFont = False
          TabOrder = 4
          Text = 'R$ 0,00'
          OnClick = EditValorClick
          OnEnter = EditValorEnter
          OnExit = EditValorExit
        end
      end
      object GroupBox2: TGroupBox
        Left = 32
        Top = 280
        Width = 705
        Height = 81
        ParentBackground = False
        TabOrder = 2
        object Label6: TLabel
          Left = 13
          Top = 29
          Width = 103
          Height = 18
          Caption = 'Plano de Contas:'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Trebuchet MS'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object btnAdicionar: TSpeedButton
          Left = 576
          Top = 23
          Width = 25
          Height = 27
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFFFFFF00}
          OnClick = btnAdicionarClick
        end
        object cbPlanoContas: TComboBox
          Left = 120
          Top = 24
          Width = 457
          Height = 26
          Ctl3D = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Trebuchet MS'
          Font.Style = [fsBold]
          ItemHeight = 18
          ParentCtl3D = False
          ParentFont = False
          TabOrder = 0
          OnEnter = cbPlanoContasEnter
          OnExit = cbPlanoContasExit
        end
      end
      object btnSalvar: TBitBtn
        Left = 512
        Top = 376
        Width = 115
        Height = 41
        Caption = '&Salvar'
        TabOrder = 3
        OnClick = btnSalvarClick
        Glyph.Data = {
          
          0019FF6F0200}
      end
      object btnCancelar: TBitBtn
        Left = 632
        Top = 376
        Width = 113
        Height = 41
        Caption = '&Cancelar'
        TabOrder = 5
        OnClick = btnCancelarClick
        Glyph.Data = {
          
          FF0000005500}
      end
      object StatusBar1: TStatusBar
        Left = 0
        Top = 440
        Width = 769
        Height = 39
        Color = clGradientActiveCaption
        Panels = <>
      end
      object editMovimento: TDateTimePicker
        Left = 116
        Top = 32
        Width = 186
        Height = 26
        Date = 44082.604031516200000000
        Time = 44082.604031516200000000
        TabOrder = 6
      end
    end
    object TabSheet2: TTabSheet
      BorderWidth = 1
      Caption = 'Transfer'#234'ncias'
      ImageIndex = 1
      object Label7: TLabel
        Left = 40
        Top = 40
        Width = 119
        Height = 16
        Caption = 'Data da Opera'#231#227'o:'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
        OnClick = btnAdicionarClick
      end
      object Label8: TLabel
        Left = 416
        Top = 40
        Width = 76
        Height = 16
        Caption = 'Documento:'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object btnSalvar2: TSpeedButton
        Left = 513
        Top = 376
        Width = 114
        Height = 41
        Cursor = crHandPoint
        Caption = '&Salvar'
        Glyph.Data = {
          
          0019FF6F0200}
        OnClick = btnSalvar2Click
      end
      object btnCancelar2: TSpeedButton
        Left = 632
        Top = 376
        Width = 113
        Height = 41
        Cursor = crHandPoint
        Caption = '&Cancelar'
        Glyph.Data = {
          
          FF0000005500}
        OnClick = btnCancelar2Click
      end
      object editDataMov: TDateTimePicker
        Left = 162
        Top = 32
        Width = 186
        Height = 26
        Date = 44092.688106886580000000
        Time = 44092.688106886580000000
        TabOrder = 0
      end
      object editDocumentacao: TEdit
        Left = 496
        Top = 33
        Width = 241
        Height = 24
        Ctl3D = False
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Trebuchet MS'
        Font.Style = [fsBold]
        ParentCtl3D = False
        ParentFont = False
        TabOrder = 1
        OnEnter = editDocumentacaoEnter
        OnExit = editDocumentacaoExit
      end
      object GroupBox3: TGroupBox
        Left = 32
        Top = 72
        Width = 705
        Height = 209
        ParentBackground = False
        TabOrder = 2
        object Label9: TLabel
          Left = 32
          Top = 37
          Width = 149
          Height = 16
          Caption = 'Conta Corrente (Sa'#237'da):'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label11: TLabel
          Left = 20
          Top = 85
          Width = 161
          Height = 16
          Caption = 'Conta Corrente (Entrada):'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object btnadcEntrada: TSpeedButton
          Left = 578
          Top = 79
          Width = 26
          Height = 26
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFFFFFF00}
          OnClick = btnadcEntradaClick
        end
        object Label10: TLabel
          Left = 396
          Top = 118
          Width = 125
          Height = 22
          Caption = 'Valor Movimento:'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -16
          Font.Name = 'Trebuchet MS'
          Font.Style = []
          ParentFont = False
        end
        object editContaSaida: TEdit
          Left = 184
          Top = 32
          Width = 393
          Height = 24
          Ctl3D = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Trebuchet MS'
          Font.Style = [fsBold]
          ParentCtl3D = False
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
          OnEnter = editContaSaidaEnter
          OnExit = editContaSaidaExit
        end
        object editContaEntrada: TEdit
          Left = 184
          Top = 80
          Width = 393
          Height = 24
          Ctl3D = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Trebuchet MS'
          Font.Style = [fsBold]
          ParentCtl3D = False
          ParentFont = False
          ReadOnly = True
          TabOrder = 1
          OnEnter = editContaEntradaEnter
          OnExit = editContaEntradaExit
        end
        object editValorMov: TEdit
          Left = 398
          Top = 139
          Width = 187
          Height = 24
          Ctl3D = False
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlue
          Font.Height = -19
          Font.Name = 'Trebuchet MS'
          Font.Style = []
          ParentCtl3D = False
          ParentFont = False
          TabOrder = 2
          Text = 'R$ 0,00'
          OnClick = editValorMovClick
          OnEnter = editValorMovEnter
          OnExit = editValorMovExit
        end
      end
      object StatusBar2: TStatusBar
        Left = 0
        Top = 440
        Width = 769
        Height = 39
        Color = clGradientActiveCaption
        Panels = <>
      end
      object BitBtn1: TBitBtn
        Left = 56
        Top = 384
        Width = 75
        Height = 25
        Caption = 'teste'
        TabOrder = 4
        OnClick = BitBtn1Click
      end
    end
  end
  object tbl_Plano: TTable
    Active = True
    DatabaseName = 'HortiFrut'
    TableName = 'tbl_PlanoContas.DB'
    Left = 404
    Top = 29
    object tbl_PlanoCod_PlanoContas: TIntegerField
      FieldName = 'Cod_PlanoContas'
    end
    object tbl_PlanoDescricao_PlanoContas: TStringField
      FieldName = 'Descricao_PlanoContas'
      Size = 60
    end
    object tbl_PlanoSequencia_PlanoContas: TStringField
      FieldName = 'Sequencia_PlanoContas'
      Size = 16
    end
    object tbl_PlanoTipo_PlanoContas: TStringField
      FieldName = 'Tipo_PlanoContas'
      Size = 16
    end
    object tbl_PlanoCategoria_PlanoContas: TStringField
      FieldName = 'Categoria_PlanoContas'
      Size = 1
    end
    object tbl_PlanoAtivo_PlanoContas: TStringField
      FieldName = 'Ativo_PlanoContas'
      Size = 1
    end
    object tbl_PlanoValorTotal_PlanoContas: TCurrencyField
      FieldName = 'ValorTotal_PlanoContas'
    end
  end
  object tbl_Movimentacoes: TTable
    DatabaseName = 'HortiFrut'
    TableName = 'tbl_Movimentacao.db'
    Left = 372
    Top = 29
    object tbl_MovimentacoesCod_Movimentacao: TAutoIncField
      FieldName = 'Cod_Movimentacao'
      ReadOnly = True
    end
    object tbl_MovimentacoesId_Movimentacao: TIntegerField
      FieldName = 'Id_Movimentacao'
    end
    object tbl_MovimentacoesData_Movimentacao: TDateField
      FieldName = 'Data_Movimentacao'
    end
    object tbl_MovimentacoesHistorico_Movimentacao: TStringField
      FieldName = 'Historico_Movimentacao'
      Size = 60
    end
    object tbl_MovimentacoesDebito_Movimentacao: TCurrencyField
      FieldName = 'Debito_Movimentacao'
    end
    object tbl_MovimentacoesCredito_Movimentacao: TCurrencyField
      FieldName = 'Credito_Movimentacao'
    end
    object tbl_MovimentacoesConciliado_Movimentacao: TStringField
      FieldName = 'Conciliado_Movimentacao'
      Size = 3
    end
    object tbl_MovimentacoesData_FinalMovimentacao: TDateField
      FieldName = 'Data_FinalMovimentacao'
    end
    object tbl_MovimentacoesExtrato_Movimentacao: TGraphicField
      FieldName = 'Extrato_Movimentacao'
      BlobType = ftGraphic
    end
    object tbl_MovimentacoesDocumento_Movimentacao: TStringField
      FieldName = 'Documento_Movimentacao'
      Size = 60
    end
    object tbl_MovimentacoesPlanoConta_Movimentacao: TStringField
      FieldName = 'PlanoConta_Movimentacao'
      Size = 60
    end
    object tbl_MovimentacoesId_PlanoContas: TIntegerField
      FieldName = 'Id_PlanoContas'
    end
  end
  object tbl_Transferencia: TTable
    DatabaseName = 'HortiFrut'
    TableName = 'tbl_Transferencia.db'
    Left = 436
    Top = 29
    object tbl_TransferenciaCod_Transferencia: TAutoIncField
      FieldName = 'Cod_Transferencia'
      ReadOnly = True
    end
    object tbl_TransferenciaData_Transferencia: TDateField
      FieldName = 'Data_Transferencia'
    end
    object tbl_TransferenciaValor_Transferencia: TCurrencyField
      FieldName = 'Valor_Transferencia'
    end
    object tbl_TransferenciaDest_Transferencia: TStringField
      FieldName = 'Dest_Transferencia'
      Size = 60
    end
    object tbl_TransferenciaReceb_Transferencia: TStringField
      FieldName = 'Receb_Transferencia'
      Size = 60
    end
    object tbl_TransferenciaDocumento_Transferencia: TStringField
      FieldName = 'Documento_Transferencia'
      Size = 60
    end
  end
  object tbl_ContasCorrentes: TTable
    DatabaseName = 'HortiFrut'
    TableName = 'tbl_ContaCorrente.DB'
    Left = 468
    Top = 29
  end
  object tbl_Planos: TTable
    DatabaseName = 'HortiFrut'
    TableName = 'tbl_Planos.db'
    Left = 500
    Top = 29
    object tbl_PlanosCod_Planos: TAutoIncField
      FieldName = 'Cod_Planos'
      ReadOnly = True
    end
    object tbl_PlanosDescricao_Planos: TStringField
      FieldName = 'Descricao_Planos'
      Size = 100
    end
    object tbl_PlanosDoc_Planos: TStringField
      FieldName = 'Doc_Planos'
      Size = 60
    end
    object tbl_PlanosValor_Planos: TCurrencyField
      FieldName = 'Valor_Planos'
    end
    object tbl_PlanosData_Planos: TDateField
      FieldName = 'Data_Planos'
    end
    object tbl_PlanosTipo_Planos: TStringField
      FieldName = 'Tipo_Planos'
      Size = 1
    end
    object tbl_PlanosId_PlanoContas: TIntegerField
      FieldName = 'Id_PlanoContas'
    end
    object tbl_PlanosId_Movimentacao: TIntegerField
      FieldName = 'Id_Movimentacao'
    end
    object tbl_Planosid_ContaCorrente: TIntegerField
      FieldName = 'id_ContaCorrente'
    end
  end
  object DataSource1: TDataSource
    DataSet = tbl_Plano
    Left = 348
    Top = 37
  end
end
