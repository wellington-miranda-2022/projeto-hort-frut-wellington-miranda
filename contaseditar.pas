...
begin
MessageDlg('Clique em Gravar para salvar as alterações!',MtInformation,[mbok],0);
exit;
end;
edit := 0;
Self.Close;
...
MessageDlg('Não a registros a serem deletados',MtInformation,[mbok],0);
end;
...
begin
MessageDlg('Campos vázios, verifique',MtWarning,[mbok],0);
exit;
end else
tbl_ContasPagar.Edit;
if edit = 1 then
...
tbl_ContasPagar.FieldByName('Documentos_ContasPagar').Value := InputBox('Número do documento','Nº:','');
end;
...
MessageDlg('Não a registros a serem editados!',MtInformation,[mbok],0);
end else
abort;
end;
...
tbl_ContasDetalhes.FieldByName('Num_DocContasDetalhes').Value := InputBox('Número do documento','Documento parcela:'+IntToStr(qtdparc),'000'+IntToStr(qtdparc));
...
begin
MessageDlg('Não a lancamentos referentes a esse período!',MtInformation,[mbok],0);
exit;
end;
...
tbl_ContasPagar.FieldByName('Pago_ContasPagar').Value := 'NÃO';
end;
